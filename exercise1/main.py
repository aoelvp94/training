...
rv = [item for item in
                    map(google_news._process, feed_data.entries[:google_news._max_results]) if item]
articles = [google_news.get_full_article(r['url']) for r in rv]
articles = [article for article in articles if article is not None]
gnews_df = pd.DataFrame([x.__dict__ for x in articles])[
        ["link_hash", "title", "text", "html", "summary", "source_url", "url", "publish_date", "meta_data"]
    ]
gnews_df['description'] = gnews_df['meta_data'].apply(lambda x: x.get('description') if x is not None else x)
gnews_df['og'] = gnews_df['meta_data'].apply(lambda x: x.get('og') if x is not None else x)
gnews_df['date_issued'] = gnews_df['meta_data'].apply(lambda x: x.get('DC.date.issued') if x is not None else x)
gnews_df['site_name'] = gnews_df['og'].apply(lambda x: x.get('site_name') if x is not None else x)
gnews_df = gnews_df.dropna(subset=['site_name'])

del gnews_df['og']
del gnews_df['meta_data']
gnews_df = gnews_df.set_index(['link_hash', 'site_name'])

engine = create_engine('postgresql+psycopg2://postgres:postgres@0.0.0.0:5432/postgresdb', echo=False)
gnews_df.to_sql('news', con=engine)
print(engine.execute("SELECT * FROM news").fetchall())
